...
void setDigiSize(sizefont size)
{
	...
	case middle:
		...
		case 320:
		case 240:
			ucg_SetFont(&ucg, ucg_font_osb29_tf);
			break;
		case 128:
			ucg_SetFont(&ucg, ucg_font_osb18_tf);
			break;
	...
	case large:
		...
		case 240:
			...
			break;
		case 128:
			ucg_SetFont(&ucg, ucg_font_osb21_hf);
			break;
		case 96:
			ucg_SetFont(&ucg, ucg_font_osb18_hf);
			break;
		case 132:
		default: // 160
			ucg_SetFont(&ucg, ucg_font_osb41_tn);
...
void removeUtf8(char *characters)
{
	...
	while (characters[Rindex])
	{
		if ((characters[Rindex] >= 0xc2) && (characters[Rindex] <= 0xc3)) // only 0 to FF ascii char
		{
			...
			if (utf8 >= 0x100)
				characters[Rindex + 1] = 0x1f; //Erase to non-printable symbol
			int sind = Rindex + 1;
...
static void drawSecond(unsigned timein)
{
	...
	if (insec != timein)
	{
		...
		ucg_SetFontMode(&ucg, UCG_FONT_MODE_SOLID);
		ucg_SetColor(&ucg, 0, CBODY);
		ucg_DrawString(&ucg, x - len - 8, yy - 21, 0, strseco);
		ucg_SetFontMode(&ucg, UCG_FONT_MODE_TRANSPARENT);
...
void drawTimeUcg(uint8_t mTscreen, unsigned timein)
{
	...
	case 1:
		...
		ucg_DrawString(&ucg, 4, yy - 21, 0, strdate);
		/* fall through */
	case 2:
		setDigiSize(text);
		if (getDdmm())
