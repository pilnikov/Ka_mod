...
void option_get_lcd_info(uint8_t *dtyp, uint8_t *drot)
{
	...
	uint8_t typ, rot;
	// init default
	*dtyp = g_device->lcd_type;
	*drot = ((g_device->options32) & T_ROTAT) ? 1 : 0;
	if (open_partition(hardware, option_space, NVS_READONLY, &hardware_handle) != ESP_OK)
	{
		ESP_LOGE(TAG, "lcd_info");
		return;
	}
	...
	if (*drot)
		*drot = 1;
	if (err != ESP_OK)
		ESP_LOGE(TAG, "oget_lcd_info err 0x%x", err);
	close_partition(hardware_handle, hardware);
}

void option_set_lcd_info(uint8_t dtyp, uint8_t drot)
{
	esp_err_t err;
	nvs_handle hardware_handle;
	...
	err = nvs_set_u8(hardware_handle, "O_LCD_TYPE", dtyp);
	err |= nvs_set_u8(hardware_handle, "O_LCD_ROTA", drot ? 1 : 0);
	if (err != ESP_OK)
