...
void Spi_init()
{
	...
	ret = spi_bus_initialize(2, &buscfg, 1); // dma
...
//		VS1053_WriteSci(SPI_CLOCKF, 0xB800); // SC_MULT = x1, SC_ADD= x1
	...
	vTaskDelay(5);
	ESP_LOGI(TAG,"volume: %d",80);
	VS1053_SetVolume( 80);	
	VS1053_SetTreble(50);
	VS1053_SetBass(254);
	VS1053_SetTrebleFreq(0);
	VS1053_SetBassFreq(254);
	VS1053_SetSpatial(1);
}
...
void VS1053_flush_cancel(uint8_t mode)
{   // 0 only fillbyte  1 before play    2 cancel play
	...
	if (mode != 0) //set CANCEL
	{
		...
		for (y = 0; y < 513; y++)
			buf[y] = endFillByte;
		for (y = 0; y < 5; y++)
			VS1053_SendMusicBytes(buf, 512); // 4*513 = 2052
	}
	else
	{
		for (y = 0; y < 5; y++)
